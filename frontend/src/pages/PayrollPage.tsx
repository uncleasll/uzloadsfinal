...
function StatusBadge({ status }: { status: string }) {
  return (
    <span className={'inline-block whitespace-nowrap rounded-full px-1.5 py-0.5 text-[10px] font-semibold ' + (SETTLEMENT_STATUS_STYLE[status] || 'bg-slate-100 text-slate-500')}>
      ...
      <div className="flex flex-shrink-0 flex-col gap-3 border-b border-slate-200/80 bg-white px-4 py-4 lg:px-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="mr-1 flex-shrink-0">
            ...
            <p className="mt-0.5 text-[11px] font-medium text-slate-400">Driver settlements, balances and payments</p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative" ref={batchRef}>
            <button onClick={()=>selected.length>0 && setBatchOpen(v=>!v)}
              ...
              className={'inline-flex h-9 items-center gap-1.5 rounded-lg px-3 text-[11px] font-semibold shadow-sm transition-colors ' + (selected.length>0 ? 'border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100' : 'cursor-not-allowed border border-slate-200 bg-slate-50 text-slate-400')}>
              Batch actions
              {selected.length>0 && <span className="flex h-4 min-w-4 items-center justify-center rounded-full bg-blue-600 px-1 text-[9px] font-bold text-white">{selected.length}</span>}
              ...
              <div role="menu" className="absolute left-0 top-full z-30 mt-1 w-52 overflow-hidden rounded-lg border border-slate-200 bg-white py-1 shadow-xl shadow-slate-950/10">
                {['Email settlements','Change status','Export to QuickBooks','Download attachments','Download Excel'].map(a=>(
                  <button key={a} role="menuitem" onClick={()=>handleBatch(a)}
                    className="block w-full px-3 py-2 text-left text-[11px] font-medium text-slate-700 transition-colors hover:bg-slate-50">{a}</button>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="flex min-w-0 flex-1 flex-wrap items-center justify-end gap-2">
            ...
            <div className="flex h-9 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 shadow-sm">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Balance due</span>
      ...
      {showFilter && (
        <div className="flex-shrink-0 border-b border-slate-200/80 bg-slate-50/60 px-4 py-3.5 lg:px-5">
          ...
          <div className="grid grid-cols-2 gap-2.5 md:grid-cols-4">
            ...
            <div>
              <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-400">Driver</label>
              ...
              return (
                <tr key={s.id} onClick={()=>setEditId(s.id)}
                  ...
                  <td className="px-1.5 py-1">
                    <button onClick={e=>{e.stopPropagation();setEditId(s.id)}} className="font-semibold text-blue-600 hover:underline text-[11px]">
                      {s.settlement_number}
                    </button>
                  </td>
