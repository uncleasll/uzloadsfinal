...
// ZIP prefix → US state lookup
// Covers all USPS ZIP prefix ranges
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  '700':'LA','701':'LA','703':'LA','704':'LA','705':'LA','706':'LA','707':'LA','708':'LA','709':'LA',
...
export default function NewLoadModal({ onClose, onSaved, entities }: Props) {
  ...
  const lookupZip = async (zip: string, stopType: 'pickup' | 'delivery') => {
    try {
      ...
      setForm(f => ({
        ...f,
        [`${stopType}_city`]: city || f[`${stopType}_city` as keyof typeof f],
        [`${stopType}_state`]: state || f[`${stopType}_state` as keyof typeof f],
      }))
    } catch {
            ...
            <Field label="ZIP">
              <input
                ...
                onChange={e => {
                  ...
                  set('pickup_zip', zip)
                  if (zip.length === 5) {
                    lookupZip(zip, 'pickup')
                  } else if (zip.length >= 3) {
                    const state = getStateFromZip(zip)
                    if (state) set('pickup_state', state)
                  }
                }}
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <Field label="ZIP">
              <input
                ...
                onChange={e => {
                  const zip = e.target.value
                  set('delivery_zip', zip)
                  if (zip.length === 5) {
                    lookupZip(zip, 'delivery')
                  } else if (zip.length >= 3) {
                    const state = getStateFromZip(zip)
                    if (state) set('delivery_state', state)
                  }
                }}
                className="input-base text-xs"
                maxLength={10}
